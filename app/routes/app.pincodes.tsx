...
export async function action({
  ...
  try {
    if (intent === "save") {
      ...
      const existingPincode =
        await getPincodeByValue(
          shop.id,
          pincode,
        );

      /*
       * Updating an existing pincode does not consume
       * another Free-plan slot.
       */
      if (
        !isPro &&
        !existingPincode
        ...
        if (
          ...
          return data(
            {
              error:
                `You have reached the Free plan limit of ${FREE_PINCODE_LIMIT} pincodes. Upgrade to Pro to add more pincodes.`,
            },
            {
              status: 403,
            },
      ...
      return data({
        success:
          existingPincode
            ? `Pincode ${pincode} updated successfully.`
            : `Pincode ${pincode} added successfully.`,
      });
    }
    ...
    if (intent === "update") {
      ...
      /*
       * A Free-plan merchant cannot reactivate a record
       * that is inactive specifically because of the plan.
       */
      if (
        !isPro &&
        ...
        return data(
          {
            error:
              "This pincode is inactive because it exceeds the Free plan limit. Upgrade to Pro to reactivate it.",
          },
          {
            status: 403,
          },
    ...
    if (
      ...
      /*
       * If one of the first 100 records was deleted,
       * the next plan-disabled record can take its place.
       */
      await enforcePincodePlanLimit({
        shopId: shop.id,
        isPro,
    ...
    if (
      ...
      if (
        ...
        return data(
          {
            error:
              "One or more selected pincodes could not be found.",
          },
          {
            status: 404,
          },
      ...
      if (!isPro) {
        ...
        if (
          planRestricted.length >
          0
        ) {
          return data(
            {
              error:
                "One or more selected pincodes exceed the Free plan limit. Upgrade to Pro to activate them.",
            },
            {
              status: 403,
            },
      ...
      /*
       * Activate only records that are currently inactive.
       * Already active records are ignored.
       */
      const inactiveIds =
        selectedPincodes
          .filter(
      ...
      if (
        ...
        return data({
          success:
            "All selected pincodes are already active.",
        });
      }
